...
class ImprovedSkeletonNet(nn.Module):
    def __init__(self, num_classes=60, num_people=4, max_frames = 60):
        super().__init__()
        self.bodies = num_people
    ...
    def predict(self, data):
        ...
        if isinstance(data, str):
            ...
            while True:
                ...
                for joint_idx in sorted(JOINTS.keys()):
                    ...
                    keypoints_2d.append([kpt[0], kpt[1], 0])
        ...
        data = normalize_skeleton(data)
        data = interpolate_frames(data, target=self.max_frames)
...
def train(config):
    ...
    print("\nСоздание модели...")
    model = ImprovedSkeletonNet(num_classes=num_classes, num_people=config.MAX_PEOPLE, max_frames = config.MAX_FRAMES).to(config.device)
    ...
    for epoch in range(config.EPOCHS):
        ...
        with torch.no_grad():
            for data, labels in tqdm(val_loader, desc=f"Epoch {epoch+1}/{config.EPOCHS}"):
                data, labels = data.to(config.device), labels.to(config.device)
        ...
        if val_acc > best_acc:
            ...
            patience = 0
            est_acc = val_acc
            model_path = '../tests/models/conv3dcnn.pth'
...
if __name__ == "__main__":
    ...
    config_path = "../tests/conv3dcnn_params.yml"
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    config2 = Config()
    # Извлекаем параметры
    config2.SKELETON_DIR = config['data']['skeleton_dir']
    config2.BATCH_SIZE = config['training']['batch_size']
    config2.EPOCHS = config['training']['epochs']
    config2.LEARNING_RATE = config['training']['learning_rate']
    config2.WEIGHT_DECAY = config['training'].get('weight_decay', 0.01)
    config2.NUM_WORKERS = config['training'].get('num_workers', 8)
